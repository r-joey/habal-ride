<script> 
	...
	async function handleSubmit (e) {
		...
		const { success, message, riderData } = await signUp(data, 'riders');
		if (success) { 
...
<div class="space-y-6 p-4">
	<div class="text-center">
		...
		<p class="text-sm sm:text-base text-gray-600 mt-2">Join Habal-Ride as a passenger</p>
	</div>
	...
	<form class="space-y-4" onsubmit={handleSubmit}>
		<div class="space-y-2">
			...
			<label class="form-control w-full max-w-xs">
				...
				<input name="mobile_number" type="text" placeholder="Please enter your email" class="input input-bordered w-full max-w-xs" /> 
			</label>
	...
	<p class="text-center text-sm sm:text-base text-gray-600">
		...
		<a href="/r/signin" class="btn btn-ghost btn-link">Sign In</a>
	</p>
</div>
